...
int loadTrain(const char* file, float tfs[]) {
    ...
    int bc=400;//*20
    proAddR[0]=1;
    proAddR[1]=bc*5+proAddR[0];
    proAddR[2]=bc*3+proAddR[1];
    proAddR[3]=bc*2+proAddR[2];
    proAddR[4]=bc*2+proAddR[3];
    proAddR[5]=bc*2+proAddR[4];
    proAddR[6]=bc*1+proAddR[5];
    proAddR[7]=bc*1+proAddR[6];
    /*开启进程*/
    int cpus = 0;
...
//            memcpy(ttfsI,shm+startR,(recNum+1)*2*sizeof(int));
            for(int j=0;j<l;++j){
                tfsI[j]+=(shm[si]<<8)+shm[si+1];
                si+=4;
            }
//            count01[0]+=ttfsI[l];
//            count01[1]+=ttfsI[l+1];
            count01[0]+=(shm[si]<<8)+shm[si+1];
            count01[1]+=(shm[si+4]<<8)+shm[si+5];
...
//    float32x4_t f4t_tfs[(recNum>>2)];
//    for(int i=0;i<(recNum>>2);++i){
//        f4t_tfs[i]=vld1q_f32(tfs+(i<<4));//依次取4个元素存入寄存器vec
//    }
    float32x4_t f4t_sum0 = vdupq_n_f32(0.0);
    float32x4_t f4t_sum1 = vdupq_n_f32(0.0);
    float32x4_t f4t_tef,f4t_c,f4t_fts0,f4t_fts1;
    float sumb0=0,sumb1=0,pow;///////////////
    while(jd<size){
...
//        f4t_c=vsubq_f32(f4t_fts0,f4t_tef);//返回两个寄存器对应元素之差 r = a-b
//        f4t_sum0=vmlaq_f32(f4t_sum0,f4t_c,f4t_c);//计算 r = a+b*c
//        f4t_c=vsubq_f32(f4t_fts1,f4t_tef);//返回两个寄存器对应元素之差 r = a-b
//        f4t_sum1=vmlaq_f32(f4t_sum1,f4t_c,f4t_c);//计算 r = a+b*c

        ///////////
        for(int j=0;j<4;++j){
            pow=tempf[j]-tfs[jnow+j];
            sumb0+=pow*pow;
            pow=tempf[j]-tfs[jnow+recNum+j];
            sumb1+=pow*pow;
        }
        ///////////////
        jnow+=4;
