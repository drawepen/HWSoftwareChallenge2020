...
char* predict(char *testData,float *tfs,int size,int recNum){
...
//        realSize=read(fd,testData,readSize);
        //开进程
        pid_t fpid=fork();//进程号
        if (fpid < 0)
...
//    cout<<"进程"<<proId<<endl;
    ...
    char *result=predict(testData,tfs,realSize,recNum);
    //传递结果
